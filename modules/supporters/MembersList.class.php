...
class MembersList extends \WP_List_Table
{
    ...
    public function get_items($use_limit = false)
    {
        ...
        $query->tables($query->wpdb()->posts)->where(['post_author' => [$query->wpdb()->users => 'ID'], 'post_type' => 'post'], 'AND', $query->wpdb()->posts)->groupby([$query->wpdb()->users => 'ID']);

        $query->columns(['display_name', 'user_email'], $query->wpdb()->users);
        $query->columns(['expirydate'], WP_MEMBERSHIP_TABLE_SUBSCRIPTIONS, true);
        $query->columns(['COUNT(*) as posts'], $query->wpdb()->posts, true);

        return $query->offset($offset)->action('select')->query();
    }
