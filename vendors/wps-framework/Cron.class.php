...
class Cron
{
    ...
    public static function schedule_single_event($hook, $args = array(), $timestamp = 1, $clear = false): bool
    {
        ...
        if ($clear) {
            ...
            // see wp_clear_scheduled_hook($hook, $args);
    ...
    public static function unschedule_function(callable $function, array $args = array()): void
    {
        $events = get_option('cron.events', array());
    ...
    public function cron_function_hook(): void
    {
    ...
    public function set_schedule($time, $recurrence = 'daily'): bool
    {
        ...
        // offset of one day (DAY_IN_SECONDS) fixes cron running soon on save
        return wp_schedule_event(wps_time('timestamp', DAY_IN_SECONDS), $recurrence, "$this->context-cron") === true;
    }
